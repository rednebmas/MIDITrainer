...
final class MistakeQueueRepository {
    ...
    /// Inserts a new mistake into the queue and returns it.
    func insert(seed: UInt64, settings: PracticeSettingsSnapshot, sourceName: String? = nil, clearance: Int = 3, now: Date = Date()) throws -> QueuedMistake {
        try db.readWrite { handle in
            let sql = """
            INSERT INTO mistake_queue (seed, settingsJson, sourceName, currentClearance, totalFailures, questionsWaited, queuedAt)
            VALUES (?, ?, ?, ?, 1, 0, ?);
            """
    ...
    func loadAll() throws -> [QueuedMistake] {
        try db.readWrite { handle in
            let sql = """
            SELECT id, seed, settingsJson, sourceName, currentClearance, totalFailures, questionsWaited, queuedAt
            FROM mistake_queue
            ...
            while sqlite3_step(statement) == SQLITE_ROW {
                ...
                guard let jsonData = String(cString: jsonPtr).data(using: .utf8),
                      let settings = try? decoder.decode(PracticeSettingsSnapshot.self, from: jsonData) else { continue }

                let totalFailures: Int? = sqlite3_column_type(statement, 5) == SQLITE_NULL
                    ? nil
                    : Int(sqlite3_column_int(statement, 5))
                mistakes.append(QueuedMistake(
                    id: sqlite3_column_int64(statement, 0),
                    seed: UInt64(bitPattern: sqlite3_column_int64(statement, 1)),
                    settings: settings,
                    sourceName: sqlite3_column_text(statement, 3).map { String(cString: $0) },
                    currentClearance: Int(sqlite3_column_int(statement, 4)),
                    totalFailures: totalFailures,
                    questionsWaited: Int(sqlite3_column_int(statement, 6)),
                    queuedAt: Date(timeIntervalSince1970: sqlite3_column_double(statement, 7))
                ))
            }
    ...
    /// Updates a mistake's ladder state after a re-ask.
    func update(id: Int64, currentClearance: Int, totalFailures: Int?, questionsWaited: Int) throws {
        try db.readWrite { handle in
            let sql = """
            UPDATE mistake_queue
            SET currentClearance = ?, totalFailures = ?, questionsWaited = ?
            WHERE id = ?;
            """
            var statement: OpaquePointer?
            guard sqlite3_prepare_v2(handle, sql, -1, &statement, nil) == SQLITE_OK else {
            ...
            sqlite3_bind_int(statement, 1, Int32(currentClearance))
            if let totalFailures {
                sqlite3_bind_int(statement, 2, Int32(totalFailures))
            } else {
                sqlite3_bind_null(statement, 2)
            }
            sqlite3_bind_int(statement, 3, Int32(questionsWaited))
    ...
    /// Increments questionsWaited for entries, optionally excluding a specific ID.
    func incrementAllCounters(excluding excludedId: Int64? = nil) throws {
        try db.readWrite { handle in
            if let excludedId {
                let sql = "UPDATE mistake_queue SET questionsWaited = questionsWaited + 1 WHERE id != ?;"
                var statement: OpaquePointer?
                guard sqlite3_prepare_v2(handle, sql, -1, &statement, nil) == SQLITE_OK else {
                ...
                let sql = "UPDATE mistake_queue SET questionsWaited = questionsWaited + 1;"
